...
public class DataSeeder implements CommandLineRunner {
    ...
    public DataSeeder(UserRepository userRepository,
                      ...
                      AdminPolicyRepository policyRepository,
    ...
    public void run(String... args) throws Exception {
        ...
        // 2. Khởi tạo chính sách đặt chỗ mặc định nếu chưa có
        if (policyRepository.count() == 0) {
            BookingPolicy defaultPolicy = new BookingPolicy();
            defaultPolicy.setMaxBookingsPerDay(2);
            defaultPolicy.setMaxDurationMinutes(180);
            defaultPolicy.setCheckInGraceMinutes(15);
            defaultPolicy.setMaxRequestRatePerHour(10);
            defaultPolicy.setCheckInEarlyOpenMinutes(15);
            defaultPolicy.setUpdatedBy("SYSTEM_INIT");
            policyRepository.save(defaultPolicy);
            System.out.println("Đã khởi tạo chính sách đặt chỗ mặc định!");
        }

        // 3. Khởi tạo một số không gian mẫu để test thống kê nếu chưa có
        ...
        // 4. Khởi tạo một số booking mẫu để test số liệu thống kê nếu chưa có
